...
public class DragHandler : MonoBehaviour {
	...
	void Update()
	{
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			if (Physics.Raycast(ray, out hit))
			{
				...
				draggable = gameObjectTodrag.GetComponent<IDraggable>();
				gameObjectTodrag.transform.SetParent(null);
				if (draggable != null) {
					draggable.IDraggableStartedDrag();
				}
		...
		if (Input.GetMouseButton(0))
		{
			...
			{
				if (draggable != null) {
					draggable.IDraggableIsDragging();	
				}
		...
		if (Input.GetMouseButtonUp(0))
		{
			draggingMode = false;
			if (draggable != null) {
				draggable.IDraggableFinishedDragging();	
			}
	...
	public void SetDraggingObject(GameObject gameObj) {
		gameObjectTodrag = gameObj;
